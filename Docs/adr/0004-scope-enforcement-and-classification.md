...
## Amendment (2026-06-24) — the clear-out auto-reject is removed; the judge gets conversation context

Live use surfaced a failure mode the 8-topic synthetic calibration above couldn't see: **short, context-dependent replies have no standalone topical signal.** A student mid-Socratic-exchange answering "I don't really know" to the tutor's own follow-up question embeds with near-zero similarity to the session's anchor — not because it's off-topic, but because two or three words carry no topic at all on their own. Under the original band, that landed in clear-out and was silently auto-rejected with the templated redirect, with no Gemini call ever seeing it — derailing the exact one-guiding-question Socratic flow ADR-0004 was built to support, and making the gate feel, in the product owner's words, "too focused on being on scope."

**Decision:**
- **The clear-out band is removed.** Only the **in** threshold remains as the free, no-judge fast path (≥ 0.66 doc, ≥ 0.63 doc-less, unchanged). Everything below it — what used to split into "judge" and "auto-reject" — now uniformly goes to the judge. This trades the "out-of-scope costs a cheap embedding only" optimization for the clear-out slice in exchange for not silently misclassifying conversational continuations; the judge remains un-metered (ADR-0001), so the cost is latency on ambiguous messages, not Daily Allowance spend.
- **The judge is now given recent conversation context** (last 4 messages), not just the bare message and the topic. Its instructions explicitly call out direct replies to the tutor's own question, acknowledgments, "I don't know," and similar short reactions as in-scope continuations to be judged *with* the conversation, not in isolation. Prompt in `PROMPTS.md` §5.
- The **in** thresholds themselves are untouched — they're the calibrated "skip the judge, this is confidently on-topic" fast path and weren't implicated in this failure mode.

**Consequences:** more messages reach the judge (roughly everything that isn't confidently on-topic, not just the old ~5-6% borderline slice), so average per-message latency on ambiguous turns rises slightly; still bounded by the velocity rate limit. The downstream FOLLOW-UP one-guiding-question cap (ADR-0004 main decision) was already correct and needed no change — it was simply unreachable for these messages because the gate rejected them before they got there.
